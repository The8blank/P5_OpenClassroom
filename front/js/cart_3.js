  ...
  for (let i = 0; i < input.length; i++) {
    ...
    element.addEventListener("change", (e) => {
      ...
      if (element.value > 0) {
        ...
        dataProduct[i].quantity = element.value;
  ...
  for (const btn of btnSup) {
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (panier.length == 1) {
        ...
        // 40. retourne la suppréssion de la key item dans le local storage et réactualise la page
        return localStorage.removeItem("panier"), (location.href = "cart.html");
      }
      // 39. si il y a plus d'un article dans le panier
      if (panier.length > 1) {

        // 40. suprime un élément du panier à l'index qui égal au dataset du bouton 
        panier.splice(btn.dataset.btn, 1);

        // 41. définit le nouveau panier dans le local storage
        localStorage.setItem("panier", JSON.stringify(panier));

        // 42. réactualise la page
